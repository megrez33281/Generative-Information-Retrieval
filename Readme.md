...
### 實驗七：困難負樣本採樣策略探索
在確定了`microsoft/unixcoder-base`作為最佳基底模型後，嘗試探索不同的困難負樣本採樣策略，找到最能提升模型分辨能力的訓練方法  

#### 單一負樣本策略(N=1)
為每個正樣本嘗試以不同方式抽取1個負樣本組成三元祖

| 策略 (Strategy) | 本地分數 (Local Score) | 分析 (Analysis) |
| :--- | :--- | :--- |
| **從 Top 50 隨機抽樣** | **0.9600** | 提供了最佳的**多樣性**，讓模型學會應對各種困難樣本，泛化能力最強|
| 僅使用最難樣本 (Top 1) | 0.9400 | 分數下降，表明缺乏多樣性可能導致模型過度專注於特定類型的「陷阱」 |
| 從 Top 5 隨機抽樣 | 0.9400 | 同樣證明了過度專注於最難的樣本，效果不如更廣泛的隨機抽樣 |

#### 多重負樣本策略(N=4)
鑒於如果要為一個正樣本匹配多個負樣本需要對模型架構進行修改  
我有了個新的想法，與其在訓練迴圈中進行複雜的張量操作來一次性對比多個負樣本，不如在數據準備階段，為同一個 `(查詢, 正樣本)` 配對多次不同的負樣本，將其「複製」成多筆訓練資料  
這本質上是一種**數據增強**，雖然模型在單一步驟中仍只看到一個負樣本，但在整個Epoch中，同一個正樣本會被強制與多個不同的困難樣本進行對比，從而被反覆「錘鍊」，進而學會更穩健的表示  

| 策略 (Strategy) | 本地分數 (Local Score) | 分析 (Analysis) |
| :--- | :--- | :--- |
| **從 Top 50 隨機抽樣** | **0.9800** | **冠軍策略**。在多樣性的基礎上增加樣本數量，達到了新的性能高峰 |
| 分層抽樣 | 0.9800 | 結果與隨機抽樣持平，證明只要保證足夠的難度和多樣性，具體的採樣方式影響不大 |
| 只取最難的 Top 4 | 0.8600 | 分數顯著下降，決定性地證明了**多樣性遠比頂級難度更重要** |
