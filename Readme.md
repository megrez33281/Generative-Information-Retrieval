...
| 策略 (Strategy) | 本地分數 (Local Score) | Kaggle 分數 (Kaggle Score) | 分析 (Analysis) |
| :--- | :--- | :--- | :--- |
| 從 Top 50 隨機抽樣 | **0.9600** | 0.87200 | 本地分數最高，但Kaggle分數墊底。表明**過於廣泛的抽樣**（包含較容易的負樣本）雖有助於本地驗證，但在Kaggle測試集上泛化能力不足。 |
| 僅使用最難樣本 (Top 1) | 0.9400 | 0.90800 | 過度專注於最難的單一樣本，導致模型學習的「陷阱」類型太單一，雖然在Kaggle上表現尚可，但限制了模型的泛化能力。 |
| **從 Top 5 隨機抽樣** | 0.9400 | **0.92800** | **Kaggle 冠軍策略**。在**難度與多樣性之間取得了最佳平衡**。專注於一小撮最困難的樣本進行學習，效果最好。 |
...
| 策略 (Strategy) | 本地分數 (Local Score)| Kaggle 分數 (Kaggle Score)  | 分析 (Analysis) |
| :--- | :--- | :--- | :--- |
| 從 Top 50 隨機抽樣 | **0.9800** | 0.89200 | 同樣在本地表現頂尖，但在Kaggle上表現不佳，再次證明**過於寬鬆的抽樣範圍**對最終泛化效果可能是一種干擾。 |
| **分層抽樣** | **0.9800** | **0.92400** | **Kaggle 冠軍策略**。本地分數與隨機抽樣持平，但Kaggle分數更高。這表明**結構化的抽樣**（確保難、中、易的困難樣本都被抽到）能讓模型學到更全面的特徵，泛化能力更強。 |
| 只取最難的 Top 4 | 0.8600 | 0.84400 | 分數顯著下降，決定性地證明了**多樣性遠比頂級難度更重要**。讓模型只看最難的幾個樣本會導致嚴重過擬合。 |
